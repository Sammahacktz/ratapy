...
class DS18B20(Device):
    """A DS18B20 1-Wire digital temperature sensor.

        temp = DS18B20(pin=4)
        temp.celsius                   # e.g. 21.44
        temp.fahrenheit

    More accurate than a DHT (±0.5 degC, 0.0625 degC steps) and several can share
    one pin -- though RATA drives the first sensor on the pin, so give each its
    own pin for now. Needs a 4.7k pull-up from the data pin to VCC.

    Reads are instant and never block: the board converts continuously in the
    background (a conversion takes ~750 ms) and returns the most recent value, so
    a reading is at most ~1 s old. Right after registering, allow one conversion
    before the first read -- until then it raises "no reading yet".
    """

    DEVICE_TYPE: ClassVar[int] = p.DEV_DS18B20
    _ERR = -32768        # INT16_MIN: no sensor answered, or no reading yet
    ...
    def celsius(self) -> float:
        """Temperature in degrees Celsius (read fresh from the board)."""
        data = self._read_raw()
        if len(data) < 2:
            raise RataError("short DS18B20 response")
        raw = p.i16(data)
        if raw == self._ERR:
            raise RataError(
                f"{self!r}: no reading yet or sensor not found "
                "(check wiring and the 4.7k pull-up; allow ~1 s after start)"
            )
        return raw / 100
    ...
    def __repr__(self) -> str:
        return f"DS18B20(pin={self.pin})"


# The DS18S20 and DS1822 are the DS18B20's siblings in the Dallas 1-Wire family.
# The firmware reads whichever Dallas sensor is on the pin (via the DallasTemperature
# library) -- it does not care about the family code -- so to RATA they are the
# same device. These names are for readability; behaviour and wiring are identical,
# including the 4.7k pull-up. (Same "subclass the closest concrete one" pattern as
# LimitSwitch/Button and Nano/Uno.)

class DS18S20(DS18B20):
    """A DS18S20 1-Wire temperature sensor -- the DS18B20's older sibling.

        temp = DS18S20(pin=4)
        temp.celsius

    Identical to DS18B20 in RATA. The chip itself is fixed 9-bit (0.5 degC steps)
    rather than the DS18B20's selectable 12-bit, but the reading and API are the
    same; the difference is only granularity.
    """

    def __repr__(self) -> str:
        return f"DS18S20(pin={self.pin})"


class DS1822(DS18B20):
    """A DS1822 1-Wire temperature sensor -- another Dallas-family part.

        temp = DS1822(pin=4)
        temp.celsius

    Identical to DS18B20 in RATA (±2 degC accuracy rather than ±0.5, otherwise the
    same). Wiring, pull-up and API are unchanged.
    """

    def __repr__(self) -> str:
        return f"DS1822(pin={self.pin})"


class AccelReadout:
    """The g-scaling for an ADXL345, shared by both transports.

    The subclass supplies ``raw`` -> (x, y, z) signed counts; everything derived
    (per-axis g, magnitude) lives here so the Arduino-attached and Pi-attached
    ADXL345 cannot drift apart. Both are full-resolution mode, 256 LSB/g. Not a
    device on its own.
    """

    _SCALE: ClassVar[float] = 256.0       # LSB per g in full-resolution mode

    @property
    def raw(self) -> tuple[int, int, int]:
        """The three axes as raw signed counts -- provided by the subclass."""
        raise NotImplementedError

    @property
    def acceleration(self) -> tuple[float, float, float]:
        """(x, y, z) acceleration in g, read fresh."""
        x, y, z = self.raw
    ...
    def magnitude(self) -> float:
        """Total acceleration in g -- about 1.0 (gravity) when held still."""
        x, y, z = self.acceleration
        return math.sqrt(x * x + y * y + z * z)


class ADXL345(AccelReadout, Device):
    """An ADXL345 3-axis accelerometer (I2C), behind an Arduino.

    For one wired straight to the Raspberry Pi's own I2C bus, see ``PiADXL345``
    (same API, no Arduino).

        accel = ADXL345(board=board)      # default I2C address 0x53
        accel.x, accel.y, accel.z         # acceleration per axis, in g
        accel.acceleration                # (x, y, z) together
        accel.magnitude                   # total, in g (≈1.0 at rest)

    The ADXL345 sits on the **Arduino's own I2C bus** (the Arduino is master to
    it): SDA/SCL -- pins 20/21 on a Mega, A4/A5 on an Uno/Nano -- plus 3.3 V and
    GND. It takes no RATA pin; `address` is its bus address (0x53 with SDO->GND,
    0x1D with SDO->VCC).

    Reads are fresh each access (a fast I2C transaction, no ~750 ms wait like the
    DS18B20). Values are in g at 3.9 mg/LSB (the chip's full-resolution mode).

    IMPORTANT: this needs the RATA board on **serial** transport. An I2C-*slave*
    board (flashed with `--i2c`) has already claimed the one TWI peripheral to
    talk to the Pi, so it cannot also master the ADXL345 -- registering one there
    is refused by the firmware.
    """

    DEVICE_TYPE: ClassVar[int] = p.DEV_ADXL345
    _ERR = -32768        # INT16_MIN on any axis: the sensor did not answer

    def __init__(self, address: int = 0x53, board: Arduino | None = None) -> None:
    ...
    # No _pins(): the ADXL345 is on the fixed SDA/SCL lines, not a RATA pin.

    def _validate(self) -> None:
        # The Arduino has to be I2C MASTER to the ADXL345 on its own bus. If this
        # board reaches the Pi over I2C, its one TWI peripheral is already the
        # Pi's slave and cannot also master the sensor -- the firmware would NACK.
        # Catch it here, before sending, with an explanation instead of a bare
        # "NACK". (The RATA transport -- who is master of the Pi<->Arduino link --
        # is a different bus from the Arduino<->sensor one; only the I2C transport
        # collides, because it shares the peripheral.)
        super()._validate()
        from ..link import I2CLink
        if isinstance(self._board._require_link(), I2CLink):
            raise RataError(
                f"{self!r}: an ADXL345 behind an Arduino needs that Arduino on "
                "SERIAL transport. This board talks to the Pi over I2C, so its one "
                "I2C peripheral is already the Pi's slave and can't also be master "
                "to the ADXL345.\n"
                "  Fix it one of two ways:\n"
                "   - re-flash this Arduino WITHOUT --i2c (serial transport), so its "
                "I2C bus is free to drive the sensor; or\n"
                "   - wire the ADXL345 straight to the Pi and use "
                "`from ratapy.devices import PiADXL345` (the Pi masters it, no "
                "Arduino involved)."
            )
    ...
    def raw(self) -> tuple[int, int, int]:
        ...
        data = self._read_raw()
        if len(data) < 6:
            raise RataError("short ADXL345 response")
        x, y, z = p.i16(data, 0), p.i16(data, 2), p.i16(data, 4)
        if x == self._ERR and y == self._ERR and z == self._ERR:
            raise RataError(
                f"{self!r}: no answer from the accelerometer -- check SDA/SCL, "
                "3.3 V power, and the I2C address (0x53 or 0x1D)"
            )
        return x, y, z

    def __repr__(self) -> str:
        return f"ADXL345(address={self.address:#04x})"
