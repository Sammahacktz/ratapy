...
class MQ2:
    """An MQ-2 combustible-gas + smoke sensor module (analog AO + digital DO).

    Despite how it is often sold, this is **not a humidity sensor** -- it detects
    LPG, propane, methane, hydrogen and smoke. Two outputs:

        gas = MQ2(channel=0, digital_pin=4)
        gas.value       # raw 0..1023 from AO (higher = more gas)
        gas.level       # 0..100 %
        gas.alarm       # True once past the module's trim-pot threshold (DO)

    `AO` goes to an analog channel; `DO` to a digital pin. DO is optional -- omit
    `digital_pin` if you only wire AO (the threshold pot is coarse, so many people
    do). It composes an `AnalogInput` and a `DigitalInput`; no new firmware.

    The reading is **relative**, not calibrated ppm, and drifts until the heater
    settles -- give it a minute after power-on (much longer for a first-ever use).
    VCC is 5 V.

    Same board layout as the rest of the MQ-x family (MQ-3 alcohol, MQ-7 CO,
    MQ-135 air quality), so this class drives those too. On most modules DO reads
    LOW when the level crosses the pot threshold; if yours is wired the other way,
    pass `alarm_when_high=True`.
    """

    def __init__(self, channel: PinLike, digital_pin: PinLike | None = None,
                 board: Arduino | None = None, alarm_when_high: bool = False) -> None:
        self.ao = AnalogInput(channel, board)
        # The DO line is driven by the module's comparator, so no pull-up.
        self.do = DigitalInput(digital_pin, board=board) if digital_pin is not None else None
        self._alarm_when_high = alarm_when_high

    @property
    def value(self) -> int:
        """Raw analog reading, 0..1023 (higher = more gas)."""
        return self.ao.value

    @property
    def level(self) -> float:
        """Gas level as 0..100 % of full scale (relative, not ppm)."""
        return self.ao.fraction * 100

    @property
    def alarm(self) -> bool:
        """True when DO says the level is past the trim-pot threshold.

        Needs `digital_pin` wired. See `alarm_when_high` for the polarity.
        """
        if self.do is None:
            raise RataError(
                "MQ2.alarm needs the DO pin -- construct with digital_pin=..., "
                "or use .value / .level from the analog output instead"
            )
