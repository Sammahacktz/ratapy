"""PiCamera -- a Raspberry Pi Camera Module on the master, driven with Picamera2 + OpenCV.
...
Raspberry Pi. The API still reads like any other RATA device::
    ...
    rp = Raspberry()
    cam = PiCamera(board=rp)          # or PiCam(board=rp) -- same class
...
class PiCamera(LocalDevice):
    ...
    PiCamera is used as a context manager.
    """
    ...
    def stream(self, window: str = "RATA PiCamera", show_fps: bool = True) -> None:
    ...
    def __repr__(self) -> str:
        ...
        return f"PiCamera({w}x{h}@{self.framerate}fps, {state})"
