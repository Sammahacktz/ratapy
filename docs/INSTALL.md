...
- `uv sync` pulls the runtime dependencies from the lockfile:
  ...
  - `opencv-python` + `numpy` — image handling for the `PiCamera` device. These
...
Then add the AVR core and the libraries the firmware uses:
...
without them still works — only `PiCamera` / `PiNeoPixel` need them, and they are
...
# I2S audio (PiMicrophone / PiSpeaker): PortAudio system lib + the Python binding:
...
- **PiCamera** uses **Picamera2** (which needs **libcamera** from the system) plus
...
- **PiNeoPixel** uses **rpi_ws281x**, which drives the strip over PWM/PCM/SPI DMA
...
- **PiMicrophone / PiSpeaker** use **sounddevice** (PortAudio → ALSA). Beyond the
  packages above, an **I2S** mic (INMP441) or amp (MAX98357) needs a device-tree
  overlay enabled in the boot config — and this part is deliberately **manual**:

  > ⚠️ **Editing the boot overlay is not automated.** The right `dtoverlay`
  > depends on your exact board and kernel, and a wrong one can leave the Pi
  > unable to boot. RATA does **not** touch `config.txt` for audio.

  As a starting point (verify against the current Raspberry Pi audio docs for
  your OS version), in `/boot/firmware/config.txt` (or `/boot/config.txt`):

  ```ini
  # MAX98357 I2S amplifier (output):
  dtoverlay=hifiberry-dac
  # INMP441 I2S mic (input) + amp together is easiest via the voiceHAT overlay:
  # dtoverlay=googlevoicehat-soundcard
  ```

  Reboot, then confirm the card shows up: `aplay -l` (output) and `arecord -l`
  (input). The Pi has **one** I2S peripheral, so running an I2S mic *and* an I2S
  amp at once needs a single combined overlay (the voiceHAT one), not two. A plain
  **USB** mic or speaker needs none of this — it just appears as an ALSA device.
...
| `opencv-python`, `numpy` | pip (main) | `PiCamera` image handling | `uv sync` |
...
| `OneWire` | arduino lib | `DS18B20` (1-Wire bus) | `arduino-cli lib install OneWire` |
| `DallasTemperature` | arduino lib | `DS18B20` temperature | `arduino-cli lib install DallasTemperature` |
| `picamera2` (+ `python3-libcamera`) | apt (Pi) | `PiCamera` | `sudo apt install python3-picamera2 python3-libcamera` |
| `libcap-dev` | apt (Pi) | building the picamera2 stack | `sudo apt install libcap-dev` |
| `rpi_ws281x` | pip (Pi) | `PiNeoPixel` | `uv pip install rpi_ws281x` (on the Pi) |
| `libportaudio2` | apt (Pi) | `PiMicrophone`/`PiSpeaker` | `sudo apt install libportaudio2` |
| `sounddevice` | pip (Pi) | `PiMicrophone`/`PiSpeaker` | `uv pip install sounddevice` (on the Pi) |
