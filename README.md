...
6. [Devices](#6-devices)
   ...
   - [DS18B20 (1-Wire temperature)](#ds18b20-1-wire-temperature)
   ...
   - [Devices on the Pi itself](#devices-on-the-pi-itself)
...
arduino-cli lib install AccelStepper DHTStable Servo OneWire DallasTemperature
...
> Driving Arduinos needs nothing more. For **PiCamera / PiNeoPixel on a Raspberry
...
> [Devices on the Pi itself](#devices-on-the-pi-itself) and
...
A DS18B20 digital temperature sensor — more accurate than a DHT (±0.5 °C,
0.0625 °C steps), and the usual choice for anything that matters.

```python
from ratapy.devices import DS18B20

temp = DS18B20(pin=4, board=board)
print(temp.celsius, "°C")        # also temp.fahrenheit
```

Wire the data pin to your chosen pin with a **4.7 kΩ pull-up to VCC** (the 1-Wire
bus needs it; without it every read fails).

Reads are **instant and never block** — even though a DS18B20 conversion takes
~750 ms. The board converts continuously in the background and each read returns
the most recent value (at most ~1 s old), so a DS18B20 in your loop won't stall a
stepper the way a naive blocking read would. The one consequence: for the first
~1 s after you create it, before a conversion has landed, reading raises
`RataError` ("no reading yet") — the same error you get if no sensor is found, so
check your wiring if it persists.

The rest of the Dallas 1-Wire family works identically — `DS18S20` and `DS1822`
are the same device to RATA (same wiring, same API, same pull-up), so use the
name that matches your part:

```python
from ratapy.devices import DS18S20      # or DS1822

temp = DS18S20(pin=4, board=board)
temp.celsius
```

### ADXL345 (accelerometer)

A 3-axis accelerometer over I2C.

```python
from ratapy.devices import ADXL345

accel = ADXL345(board=board)        # default I2C address 0x53
accel.x, accel.y, accel.z           # per axis, in g
accel.acceleration                  # (x, y, z) together
accel.magnitude                     # total — ≈ 1.0 (gravity) held still
```

The ADXL345 lives on the **Arduino's own I2C bus** (the Arduino is master to it):
SDA/SCL — **pins 20/21 on a Mega, A4/A5 on an Uno/Nano** — plus **3.3 V** and GND.
It takes no RATA pin; the argument is its bus address (`0x53` with SDO→GND, `0x1D`
with SDO→VCC). Reads are fresh each access (a fast I2C read, no ~750 ms wait).

> No Arduino? The same sensor can wire straight to the Pi instead — see
> [Devices on the Pi itself](#devices-on-the-pi-itself).

> **Serial boards only.** An I2C-*slave* board (flashed with `--i2c`) has already
> claimed its one I2C peripheral to talk to the Pi, so it can't also master the
> ADXL345 — registering one there is refused. Use a board on serial transport.
...
| `MQ2(channel, digital_pin=)` | AnalogInput + DigitalInput | `value`, `level`, `alarm` — gas/smoke (MQ-x family), *not* humidity |
...
**Gas & smoke: `MQ2`.** An MQ-2 module has two outputs — an analog level (AO) and
a digital threshold trip (DO) set by its onboard trim-pot — so RATA composes it
from an `AnalogInput` and a `DigitalInput`:

```python
from ratapy.devices import MQ2

gas = MQ2(channel=0, digital_pin=4, board=board)
gas.value        # raw 0..1023 (higher = more gas)
gas.level        # 0..100 %
gas.alarm        # True once past the trim-pot threshold (needs digital_pin)
```

Wire `AO` to an analog channel and `DO` to a digital pin (omit `digital_pin` to
use AO only). Despite how these are often sold, it is **not a humidity sensor** —
it senses combustible gas (LPG, propane, methane, hydrogen) and smoke, as a
*relative* level, not calibrated ppm, and it needs a minute of warm-up. The same
class fits the rest of the MQ-x family (MQ-3 alcohol, MQ-7 CO, MQ-135 air
quality). If your module's DO reads the opposite way, pass `alarm_when_high=True`.
...
API is the same; you just pass the Raspberry as the board. These carry a **`Pi`
prefix** so it's always clear a device runs on the Pi, not behind an Arduino:
...
cam = PiCam(board=rp)                  # Pi Camera (Picamera2 + OpenCV)
...
strip = PiNeoPixel(count=30, board=rp) # WS2812 strip on GPIO18 (one data pin)
...
An **ADXL345** accelerometer can go here too — wired to the Pi's own I2C bus
instead of behind an Arduino. Same sensor, same reading API as [the Arduino
version](#adxl345-accelerometer); the `Pi` prefix is which one you mean:

```python
from ratapy.devices import PiADXL345    # the Pi-attached one
# vs. `from ratapy.devices import ADXL345`   (behind an Arduino)

accel = PiADXL345(board=rp)           # SDA=GPIO2, SCL=GPIO3, 3.3V, GND
accel.x, accel.y, accel.z             # g; also .acceleration, .magnitude
```

Unlike the camera and strip, this needs **no `--pi` libraries** — it uses
`smbus2`, which RATA already depends on. It does need the Pi's I2C bus enabled
(`rata i2c`, then reboot), the same one-time step as driving Arduinos over I2C.
Address is `0x53` (default) or `0x1D`; pass `bus=` if it's not on `/dev/i2c-1`.

**Audio — `PiMicrophone` and `PiSpeaker`** capture and play sound through the Pi
(an INMP441 I2S mic, a MAX98357 I2S amp + speaker, or any USB audio device):

```python
from ratapy.devices import PiMicrophone, PiSpeaker

mic = PiMicrophone(board=rp)         # e.g. an INMP441
spk = PiSpeaker(board=rp)            # e.g. a MAX98357 + 3 W speaker

clip = mic.record(seconds=3)         # -> numpy float32 samples
spk.play(clip)                       # play them back
spk.tone(440, 0.5)                   # or a 440 Hz beep

mic.record_wav("note.wav", 3)        # straight to a .wav
spk.play("note.wav")                 # play a .wav
```

These use **sounddevice** (added by `bash install.sh --pi`). An **I2S** mic or amp
also needs a device-tree overlay in the boot config — a **manual, board-specific**
step RATA deliberately doesn't automate (a wrong overlay can stop the Pi booting);
see [docs/INSTALL.md](docs/INSTALL.md). A **USB** mic/speaker needs no overlay. The
...
DS18B20(pin=4).celsius                       # -> degC (1-Wire; also DS18S20, DS1822)
ADXL345(address=0x53).acceleration           # -> (x, y, z) g; .x/.y/.z, .magnitude (serial only)
PiADXL345(board=rp).acceleration             # ...or the ADXL345 wired straight to the Pi
MQ2(channel=0, digital_pin=4).level          # -> gas 0..100 %; .value (raw), .alarm (DO)

# --- on the Raspberry Pi itself (Pi* prefix; board=rp) ---
PiCam(board=rp).snapshot("p.jpg")            # camera (Picamera2 + OpenCV)
