...
# OneWire + DallasTemperature drive the DS18B20 temperature sensor.
ONEWIRE_VERSION="${ONEWIRE_VERSION:-2.3.8}"
DALLAS_VERSION="${DALLAS_VERSION:-4.0.6}"
...
install_arduino_deps() {
    ...
    "$ACLI" lib install "AccelStepper@$ACCELSTEPPER_VERSION" "DHTStable@$DHTSTABLE_VERSION" \
                        "Servo@$SERVO_VERSION" "OneWire@$ONEWIRE_VERSION" \
                        "DallasTemperature@$DALLAS_VERSION"
    ok "arduino:avr@$AVR_CORE_VERSION, AccelStepper, DHTStable, Servo, OneWire, DallasTemperature"
}
